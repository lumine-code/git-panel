...
export default function ({ confirm, notificationManager, workspace }) {
    ...
    async (next, repository, branchName, options) => {
      if (options.force) {
        const choice = await confirm({
          ...
          detail: "This operation could result in losing data on the remote.",
  ...
  checkoutPipeline.addMiddleware(
    "set-checkout-in-progress",
    async (next, repository, branchName) => {
  ...
  commitPipeline.addMiddleware("confirm-commit", async (next, repository) => {
    ...
    if (commitMessageEditors.length > 0) {
      if (
        !commitMessageEditors.some((e) => e.getFileState() !== FileState.UNMODIFIED) ||
        (await confirmCommit())
      ) {
        await next();
