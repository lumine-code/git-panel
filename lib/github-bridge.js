...
// internals, and gets `GitError`/`filterPatch` from the `lumine` module directly.
...
export default function createGitHubBridge(pack) {
  return {
    ...
    // The panel's active-repository context mirror + Git tab/dialog control.
    ...
    clone: (remoteUrl, projectPath, sourceRemoteName) =>
      pack.clone(remoteUrl, projectPath, sourceRemoteName),
