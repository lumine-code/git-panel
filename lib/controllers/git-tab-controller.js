...
export default class GitTabController extends React.Component {
  ...
  async stageFiles(filePaths) {
    ...
    const mergeMarkers = await Promise.all(
      filePaths.map(async (filePath) => {
        ...
        return {
          ...
          stagingReady: this.props.resolutionProgress.isStagingReady(fullPath),
    ...
    for (const { filePath, isMergeConflict, stagingReady, hasMarkers } of mergeMarkers) {
      if (isMergeConflict && (!stagingReady || hasMarkers)) {
