  ...
  renderCommands() {
    return (
      <Commands registry={this.props.commands} target="lumine-workspace">
        ...
        />
        <Command
          command="git-panel:show-cache-diagnostics"
        ...
        />
        <Command
          command="git-panel:fetch"
          description="Fetch from the remote without changing the working tree."
          callback={() => fetchRepository(this.props.repository)}
        />
        <Command
          command="git-panel:pull"
          description="Fetch from the remote and merge it into this branch."
          callback={() => pullRepository(this.props.repository)}
        />
        <Command
          command="git-panel:push"
          description="Send this branch's commits to its remote counterpart."
          callback={() => pushRepository(this.props.repository)}
        />
        <Command
          command="git-panel:force-push"
          description="Overwrite the remote branch with this one."
          callback={() => pushRepository(this.props.repository, { force: true })}
        />
      </Commands>
    );
  }
