...
export default class GitRootController extends React.Component {
  ...
  renderCommands() {
    ...
    return (
      <Commands registry={this.props.commands} target="atom-workspace">
        {devMode && (
          <Command
            command="git-panel:install-react-dev-tools"
        ...
        <Command
          command="git-panel:toggle-git-tab-focus"
          callback={this.props.gitTabTracker.toggleFocus}
        />
  ...
  async installReactDevTools() {
    ...
    this.props.notificationManager.addSuccess(
      "🌈 Reload your window to start using the React/Relay dev tools!",
    );
  }
  ...
  async installExtension(id) {
    ...
    await new Promise((resolve, reject) => {
      unzip(extensionFile, extensionFolder, async (err) => {
        if (
          err &&
          !(await fs.access(path.join(extensionFolder, "manifest.json")).then(
            () => true,
            () => false,
          ))
        ) {
    ...
    await fs.mkdir(extensionFolder, { recursive: true, mode: 0o755 });
