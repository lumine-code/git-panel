...
export default class EditorConflictController extends React.Component {
  constructor(props, context) {
    ...
    this.commandTarget = props.editor.getElement();
  ...
  componentDidMount() {
    ...
    this.subscriptions.add(
      this.props.editor.onDidDestroy(() => this.releaseBufferProgress()),
      buffer.onDidReload(() => this.reparseConflicts()),
      buffer.onDidStopChanging(() => this.reparseConflicts()),
      buffer.onDidSave(() => {
        this.props.resolutionProgress.markResolutionSelected(this.props.editor.getPath());
        this.reparseConflicts();
        this.props.refreshResolutionProgress(this.props.editor.getPath());
      }),
    );
    ...
    return (
      <div>
        {this.state.conflicts.size > 0 && (
          <Commands registry={this.props.commands} target={this.commandTarget}>
            <Command command="git-panel:resolve-as-ours" callback={this.getResolverUsing([OURS])} />
            <Command
              command="git-panel:resolve-as-theirs"
              callback={this.getResolverUsing([THEIRS])}
            />
            <Command command="git-panel:resolve-as-base" callback={this.getResolverUsing([BASE])} />
            <Command
              command="git-panel:resolve-as-ours-then-theirs"
              callback={this.getResolverUsing([OURS, THEIRS])}
            />
            <Command
              command="git-panel:resolve-as-theirs-then-ours"
              callback={this.getResolverUsing([THEIRS, OURS])}
            />
            <Command
              command="git-panel:resolve-as-current"
              description="Resolve this conflict by keeping the side the cursor is in."
              callback={this.resolveAsCurrent}
            />
            <Command
              command="git-panel:revert-conflict-modifications"
              description="Put the conflict markers back as Git wrote them."
              description="Put the conflict markers back as Git wrote them."
              callback={this.revertConflictModifications}
            />
            <Command command="git-panel:dismiss-conflict" callback={this.dismissCurrent} />
          </Commands>
        )}
        {Array.from(this.state.conflicts)
          .filter((conflict) => !this.state.dismissedConflicts.has(conflict))
          .map((conflict) => (
            <ConflictController
              key={conflict.getKey()}
              editor={this.props.editor}
              conflict={conflict}
              resolveAsSequence={(sources) => this.resolveAsSequence(conflict, sources)}
              dismiss={() => this.dismissConflicts([conflict])}
            />
          ))}
      </div>
    );
  ...
  componentWillUnmount() {
    ...
    this.releaseBufferProgress();
    this.layer.destroy();
  }

  componentDidUpdate(prevProps) {
    if (prevProps.isRebase !== this.props.isRebase) {
      this.reparseConflicts();
    }
  }
  ...
  getCurrentConflicts() {
    ...
    while (!currentCursor.done && !currentConflict.done) {
      ...
      while (
        !currentConflict.done &&
        !currentCursor.done &&
        currentConflict.value.getRange().start.compare(currentCursor.value) <= 0
      ) {
  ...
  getResolverUsing(sequence) {
    return () => {
      this.getCurrentConflicts().forEach((match) =>
        this.resolveAsSequence(match.conflict, sequence),
      );
    };
  }
  ...
  dismissConflicts(conflicts) {
    this.setState((prevState) => {
      const dismissedConflicts = new Set(prevState.dismissedConflicts);
      conflicts.forEach((conflict) => dismissedConflicts.add(conflict));
  ...
  resolveAsSequence(conflict, sources) {
    ...
    this.props.editor.transact(() => {
      ...
      this.innerResolveAs(conflict, firstSide.getSource());
    });
    return true;
  }

  resolveAs(conflict, source) {
    if (!conflict.getSide(source)) {
      return false;
    }
    this.props.editor.transact(() => {
  ...
  reparseConflicts() {
    ...
    const newConflicts = new Set(
      Conflict.allFromEditor(this.props.editor, this.layer, this.props.isRebase),
    );
    this.setState({ conflicts: newConflicts, dismissedConflicts: new Set() });
  }

  releaseBufferProgress() {
    if (this.bufferProgressReleased) return;
    this.bufferProgressReleased = true;
    const filePath = this.props.editor.getPath();
    this.props.resolutionProgress.clearBuffer(filePath);
    this.props.refreshResolutionProgress(filePath);
  }

  updateMarkerCount() {
    this.props.resolutionProgress.reportBufferMarkerCount(
      this.props.editor.getPath(),
      Array.from(this.state.conflicts, (c) => !c.isResolved()).filter((b) => b).length,
      { modified: this.props.editor.getFileState() !== FileState.UNMODIFIED },
    );
  }
}
