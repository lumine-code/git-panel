...
import { autobind, getCommitMessagePath } from "../helpers";
...
export default class CommitController extends React.Component {
  ...
  constructor(props, context) {
    ...
    this.props.grammars.assignLanguageMode(this.commitMessageBuffer, "source.gfm");
  }
        ...
        if (
          ...
          try {
            this.commitMessageBuffer.setText(
              await fs.readFile(this.getCommitMessagePath(), { encoding: "utf8" }),
            );
          } catch (e) {
            if (e.code !== "ENOENT") {
              throw e;
  ...
  render() {
    ...
    return (
      <CommitView
        ...
        isFetching={operationStates.isFetchInProgress()}
        isPulling={operationStates.isPullInProgress()}
        isPushing={operationStates.isPushInProgress()}
  ...
  commit(message, coAuthors = [], amend = false) {
    ...
    if (this.isCommitMessageEditorExpanded()) {
      ...
      const wrapMessage = this.props.config.get("git-panel.automaticCommitMessageWrapping");
  ...
  getCommitMessagePath() {
    return getCommitMessagePath(this.props.repository);
  }
  ...
  async toggleExpandedCommitMessageEditor(messageFromBox) {
    if (this.isCommitMessageEditorExpanded()) {
      if (this.commitMessageEditorIsInForeground()) {
        ...
        await this.activateCommitMessageEditor();
      }
    } else {
  ...
  activateCommitMessageEditor() {
    const editor = this.getCommitMessageEditors()[0];
    if (!editor) return Promise.resolve();
    return this.props.workspace.open(editor, { searchAllPanes: true });
  }
