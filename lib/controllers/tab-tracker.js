...
export default class TabTracker {
  ...
  async toggle() {
    ...
    const activeSurface = workspace.getActiveWindowSurface();
    const focusToRestore =
      activeSurface?.document.activeElement ||
      this.getDOMElement()?.ownerDocument.activeElement ||
      workspace.getElement().ownerDocument.activeElement ||
      null;
