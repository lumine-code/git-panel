...
export default class GitTabHeaderController extends React.Component {
  ...
  // The selected repository follows atom.repositories directly, so switching
  // through git-switcher or by changing the active pane item updates the
  ...
  getActiveWorkDir() {
    try {
      return (
        atom.repositories.getActiveRepositoryContext().workingDirectory || this.props.currentWorkDir
      );
    } catch {
