...
export default class ConflictController extends React.Component {
  ...
  showResolveMenu(event) {
    ...
    const commands = {
      "git-panel:resolve-as-ours": {
        description: "Resolve this conflict by keeping the side already here.",
        didDispatch: () => this.resolveAsSequence([OURS]),
      },
      "git-panel:resolve-as-base": {
        description: "Resolve this conflict by keeping the common ancestor's text.",
        didDispatch: () => this.resolveAsSequence([BASE]),
      },
      "git-panel:resolve-as-theirs": {
        description: "Resolve this conflict by keeping the side being merged in.",
        didDispatch: () => this.resolveAsSequence([THEIRS]),
      },
      "git-panel:resolve-as-ours-then-theirs": {
        description: "Keep both sides of this conflict, ours written first.",
        didDispatch: () => this.resolveAsSequence([OURS, THEIRS]),
      },
      "git-panel:resolve-as-theirs-then-ours": {
        description: "Keep both sides of this conflict, theirs written first.",
        didDispatch: () => this.resolveAsSequence([THEIRS, OURS]),
      },
      "git-panel:dismiss-conflict": {
        description: "Hide this conflict's banner without resolving anything.",
        didDispatch: () => this.props.dismiss(),
      },
    };
    this.commandSubs.add(lumine.commands.add(target, commands));
