...
export class RecentCommitView extends React.Component {
  constructor(props) {
    super(props);
    ...
    return (
      ...
      >
        <Commands registry={this.props.commands} target={this.refRoot}>
          <Command
            command="git-panel:copy-commit-sha"
            description="Copy the selected commit's full hash to the clipboard."
            callback={this.copyCommitSha}
          />
          <Command
            command="git-panel:copy-commit-subject"
            description="Copy the selected commit's subject line."
            callback={this.copyCommitSubject}
          />
          <Command
            command="git-panel:checkout-to-commit"
            description="Check out the selected commit, leaving the branch behind."
            callback={this.checkoutToCommit}
          />
        </Commands>
        {this.renderAuthors()}
  ...
  checkoutToCommit = (event) => {
    event.stopPropagation();
    this.props.checkout(this.props.commit.getSha());
  };
}
...
export default class RecentCommitsView extends React.Component {
  ...
  constructor(props) {
    super(props);
    this.refRoot = new RefHolder();
    this.loadMoreRequestedAtCommitCount = null;
  }
  ...
  render() {
    return (
      <div
        className="git-panel-RecentCommits"
        tabIndex="-1"
        ref={this.refRoot.setter}
        onScroll={this.loadMoreCommitsAtScrollEnd}
      >
        <Commands registry={this.props.commands} target={this.refRoot}>
          ...
          <Command
            command="git-panel:dive"
            description="Open the selected commit and show what it changed."
            callback={this.openSelectedCommit}
          />
          <Command
            command="git-panel:undo-last-commit"
            description="Undo the last commit, keeping its changes staged."
            callback={this.undoLastCommit}
          />
        </Commands>
        {this.renderCommits()}
      </div>
          ...
          {this.props.commits.map((commit, i) => {
            return (
              <RecentCommitView
                ...
                onMouseDown={(event) => this.mousedownOnCommit(event, commit)}
  ...
  mousedownOnCommit(event, commit) {
    if (event.button !== 1) {
      return;
    }
    ...
    event.preventDefault();
    event.stopPropagation();
    this.props.selectCommit(commit.getSha());
    this.setFocus(this.constructor.focus.RECENT_COMMIT);
  }

  loadMoreCommitsAtScrollEnd = (event) => {
    if (!this.props.hasMoreCommits || !this.props.loadMoreCommits) {
      return;
    }

    if (this.loadMoreRequestedAtCommitCount === this.props.commits.length) {
      return;
    }
    ...
    if (scrollBottom <= 8) {
      this.loadMoreRequestedAtCommitCount = this.props.commits.length;
      this.props.loadMoreCommits();
    }
  };
