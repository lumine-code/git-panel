...
export default class MergeConflictListItemView extends React.Component {
  ...
  render() {
    ...
    delete others.resolutionStatus;
  ...
  renderRemainingConflicts() {
    const status = this.props.resolutionStatus;
    if (status?.ready) {
      return (
        <span className="icon icon-check git-panel-RemainingConflicts text-success">ready</span>
      );
    } else if (status?.reason === "unsaved") {
      return <span className="git-panel-RemainingConflicts text-warning">save before staging</span>;
    } else if (status?.reason === "choice") {
      return <span className="git-panel-RemainingConflicts text-warning">choose a resolution</span>;
    } else if (status?.remaining !== undefined) {
      const pluralConflicts = status.remaining === 1 ? "" : "s";

      return (
        <span className="git-panel-RemainingConflicts text-warning">
          {status.remaining} conflict{pluralConflicts} remaining
        </span>
      );
    } else {
