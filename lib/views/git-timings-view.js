...
const genArrayCache = new Map();
const genArray = function genArray(interval, count) {
  const key = `${interval}:${count}`;
  if (genArrayCache.has(key)) {
    return genArrayCache.get(key);
  }
  const arr = [];
  for (let i = 1; i <= count; i++) {
    arr.push(interval * i);
  }
  genArrayCache.set(key, arr);
  return arr;
};
...
class MarkerSpan extends React.Component {
  ...
  handleMouseOver(e) {
    const elem = this.element.ownerDocument.createElement("div");
    this._tooltipRoot = createRoot(elem);
    this._tooltipRoot.render(<MarkerTooltip marker={this.props.marker} />);
    ...
    return (
      <div className="waterfall-timeline">
        ...
        {this.state.timelineMarks.map((time) => {
          ...
          return (
            <span className="waterfall-timeline-label" style={style} key={`tl:${time}`}>
              {time}ms
            </span>
          );
        })}
...
class WaterfallWidget extends React.Component {
      ...
      <div className="waterfall-widget inset-pannel">
        <div className="waterfall-header">
          ...
          <div className="waterfall-header-controls">
            <button className="waterfall-export-button btn btn-sm" onClick={this.handleExportClick}>
              Export
            </button>
            <Octicon icon="search" />
            <input
  ...
  async handleExportClick(e) {
    ...
    const { filePath } = await lumine.window.showSaveDialog({
...
export default class GitTimingsView extends React.Component {
  static uriPattern = "lumine-github://debug/timings";
  ...
  render() {
    return (
      <div className="git-panel-GitTimingsView">
        <div className="git-panel-GitTimingsView-header">
          <button className="import-button btn" onClick={this.handleImportClick}>
            Import
          </button>
        </div>
  ...
  async handleImportClick(e) {
    e.preventDefault();
    const domDocument = e.currentTarget.ownerDocument;
    const filePaths = await new Promise((resolve) => {
      const input = domDocument.createElement("input");
      input.type = "file";
      input.accept = ".json";
      input.addEventListener("change", () => {
        resolve(Array.from(input.files).map((f) => f.path));
      });
    ...
    try {
      ...
      lumine.notifications.addError(`Could not import timings from ${filename}`);
    }
  }
