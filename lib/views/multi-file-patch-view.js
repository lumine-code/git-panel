...
export default class MultiFilePatchView extends React.Component {
  ...
  renderCommands() {
    if (this.props.itemType === CommitDetailItem) {
      return (
        <Commands registry={this.props.commands} target={this.refRoot}>
          <Command command="git-panel:select-next-hunk" callback={this.selectNextHunk} />
          <Command command="git-panel:select-previous-hunk" callback={this.selectPreviousHunk} />
          <Command
    ...
    if (this.props.multiFilePatch.didAnyChangeExecutableMode()) {
      const command =
        this.props.stagingStatus === "unstaged"
          ? "git-panel:stage-file-mode-change"
          : "git-panel:unstage-file-mode-change";
    ...
    if (this.props.multiFilePatch.anyHaveTypechange()) {
      const command =
        this.props.stagingStatus === "unstaged"
          ? "git-panel:stage-symlink-change"
          : "git-panel:unstage-symlink-change";
    ...
    return (
      <Commands registry={this.props.commands} target={this.refRoot}>
        ...
        />
        <Command command="git-panel:jump-to-file" callback={this.didOpenFile} />
        <Command command="git-panel:surface" callback={this.props.surface} />
        <Command
          command="git-panel:toggle-patch-selection-mode"
          callback={this.didToggleSelectionMode}
        />
        ...
        for (const region of hunk.getRegions()) {
          if (region.isDeletion()) {
            ...
            for (let i = 0; i < pairCount; i++) {
              this.computeWordDiffRanges(buffer, pendingDelRows[i], addRows[i], deletedRanges, addedRanges);
            }
  ...
  computeWordDiffRanges(buffer, delRow, addRow, deletedRanges, addedRanges) {
    ...
    for (const change of changes) {
      if (change.removed) {
        deletedRanges.push([[delRow, delCol], [delRow, delCol + change.value.length]]);
        delCol += change.value.length;
      } else if (change.added) {
        addedRanges.push([[addRow, addCol], [addRow, addCol + change.value.length]]);
  ...
  renderWordHighlights(ranges, className) {
    ...
    return (
      <MarkerLayer>
        {ranges.map((range, i) => (
          <Marker key={`word-${className}-${i}`} bufferRange={Range.fromObject(range)} invalidate="never" />
        ))}
