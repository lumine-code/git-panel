...
export default class MultiFilePatchView extends React.Component {
  ...
  componentDidMount() {
    ...
    this.subs.add(
      this.props.config.onDidChange("git-panel.showDiffIconGutter", () => this.forceUpdate()),
    );
  ...
  bindSurfaceWindow() {
    ...
    this.surfaceWindow.addEventListener("mouseup", this.didMouseUp);
  }

  unbindSurfaceWindow() {
    this.surfaceWindow?.removeEventListener("mouseup", this.didMouseUp);
    this.surfaceWindow = null;
  }
    ...
    return (
      ...
      >
        ...
        <Gutter
          ...
          onMouseMove={this.didMouseMoveOnLineNumber}
        />
        <Gutter name="git-panel-comment-icon" priority={3} className="comment" type="decorated" />
        ...
        <MarkerLayer>
          {filePatch.getHunks().map((hunk, index) => {
            const containsSelection =
              this.props.selectionMode === "line" && selectedHunks.has(hunk);
  ...
  renderWordDiffDecorations() {
    const buffer = this.props.multiFilePatch.getBuffer();
    const deletedRanges = [];
    const addedRanges = [];

    for (const filePatch of this.props.multiFilePatch.getFilePatches()) {
      if (!filePatch.getRenderStatus().isVisible()) continue;
      ...
      for (const hunk of filePatch.getHunks()) {
        ...
        for (const region of hunk.getRegions()) {
          if (region.isDeletion()) {
            pendingDelRows.push(...region.getBufferRows());
          } else if (region.isAddition()) {
            const addRows = region.getBufferRows();
            const pairCount = Math.min(pendingDelRows.length, addRows.length);

            for (let i = 0; i < pairCount; i++) {
              this.computeWordDiffRanges(
                buffer,
                pendingDelRows[i],
                addRows[i],
                deletedRanges,
    ...
    if (deletedRanges.length === 0 && addedRanges.length === 0) return null;

    return (
      <Fragment>
        {this.renderWordHighlights(deletedRanges, "git-panel-FilePatchView-word--deleted")}
        {this.renderWordHighlights(addedRanges, "git-panel-FilePatchView-word--added")}
      </Fragment>
    );
  }

  computeWordDiffRanges(buffer, delRow, addRow, deletedRanges, addedRanges) {
    const delText = buffer.lineForRow(delRow);
    const addText = buffer.lineForRow(addRow);
    const changes = diffWords(delText, addText);
    ...
    for (const change of changes) {
      if (change.removed) {
        deletedRanges.push([
          [delRow, delCol],
          [delRow, delCol + change.value.length],
        ]);
        delCol += change.value.length;
      } else if (change.added) {
        addedRanges.push([
          [addRow, addCol],
          [addRow, addCol + change.value.length],
        ]);
        addCol += change.value.length;
      } else {
        delCol += change.value.length;
        addCol += change.value.length;
      }
  ...
  renderWordHighlights(ranges, className) {
      ...
      <MarkerLayer>
        {ranges.map((range, i) => (
          <Marker
            ...
            bufferRange={Range.fromObject(range)}
            invalidate="never"
          />
        ))}
    ...
    return (
      <MarkerLayer handleLayer={holder.setter}>
        {ranges.map((range, index) => {
          return (
            <Marker key={`line-${lineClass}-${index}`} bufferRange={range} invalidate="never" />
          );
        })}
