...
export default class MultiFilePatchView extends React.Component {
  ...
  renderWordDiffDecorations() {
    ...
    for (const filePatch of this.props.multiFilePatch.getFilePatches()) {
      ...
      for (const hunk of filePatch.getHunks()) {
        ...
        for (const region of hunk.getRegions()) {
          markerIds.push(region.getMarker().id);
    ...
    // The patch buffer is deliberately reused between repository refreshes.
    // Replacing its text transforms every marker layer attached to the buffer,
    // including these view-owned word highlights. Region marker IDs change
    // whenever the patch model is rebuilt, so key the layers by those IDs to
    // destroy the transformed layers and recreate them from the new ranges.
    const markerRevision = markerIds.join(":");
  ...
  renderWordHighlights(ranges, className, markerRevision) {
    if (ranges.length === 0) return null;
    return (
      <MarkerLayer key={`${className}-${markerRevision}`}>
        {ranges.map((range, i) => (
          <Marker
