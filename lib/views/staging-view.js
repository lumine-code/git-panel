...
import { autobind, fileExists, showContextMenu } from "../helpers";
...
export default class StagingView extends React.Component {
  ...
  componentDidMount() {
    this.bindSurfaceWindow();
    this.subs.add(
      this.props.workspace.addWindowSurfaceTransitionObserver(this.observeSurfaceTransition),
    ...
    return (
      <Fragment>
        <Commands registry={this.props.commands} target=".git-panel-StagingView">
          ...
          <Command
            command="git-panel:show-diff-view"
            description="Open the diff of whatever is selected in the list."
            callback={this.showDiffView}
          />
          <Command
            command="git-panel:focus-diff-view"
            description="Move focus into the diff beside the file list."
          ...
          <Command
            command="git-panel:activate-next-list"
            description="Move the selection to the next list in the panel."
            callback={this.activateNextList}
          />
          <Command
            command="git-panel:activate-previous-list"
            description="Move the selection to the previous list in the panel."
            description="Move the selection to the previous list in the panel."
          ...
          <Command
            ...
            description="Resolve every conflict in this file by keeping our side."
            callback={this.resolveCurrentAsOurs}
          />
          <Command
            command="git-panel:resolve-file-as-theirs"
            description="Resolve every conflict in this file by keeping their side."
            description="Resolve every conflict in this file by keeping their side."
            callback={this.resolveCurrentAsTheirs}
          />
          <Command
            command="git-panel:discard-changes-in-selected-files"
            description="Throw away the unstaged changes in the selected files."
            description="Throw away the unstaged changes in the selected files."
        ...
        <Commands registry={this.props.commands} target="lumine-workspace">
          <Command
            command="git-panel:stage-all-changes"
            description="Move every working-tree change into the index."
            callback={this.stageAll}
          />
          <Command
            command="git-panel:unstage-all-changes"
            description="Take every staged change back out of the index."
            callback={this.unstageAll}
          />
          <Command
            command="git-panel:discard-all-changes"
            description="Throw away every unstaged change in the working tree."
            callback={this.discardAllFromCommand}
          />
          <Command
            command="git-panel:undo-last-discard-in-git-tab"
            description="Bring back the changes the last discard threw away."
            description="Bring back the changes the last discard threw away."
  ...
  renderMergeConflicts() {
    ...
    if (mergeConflicts && mergeConflicts.length > 0) {
      ...
      const anyNotReady = mergeConflicts
        .map((conflict) => path.join(this.props.workingDirectoryPath, conflict.filePath))
        .some((conflictPath) => !resolutionProgress.isStagingReady(conflictPath));
      const anyUnresolved = mergeConflicts
        .map((conflict) => path.join(this.props.workingDirectoryPath, conflict.filePath))
        .some((conflictPath) => {
          const status = resolutionProgress.getStatus(conflictPath);
          return !status.ready && status.reason !== "unsaved";
        });
            ...
            {mergeConflicts.map((mergeConflict) => {
              ...
              return (
                <MergeConflictListItemView
                  ...
                  resolutionStatus={resolutionProgress.getStatus(fullPath)}
                  registerItemElement={this.registerItemElement}
  ...
  componentWillUnmount() {
    this.unbindSurfaceWindow();
    this.subs.dispose();
    if (this.menuCommandSubs) this.menuCommandSubs.dispose();
  }

  bindSurfaceWindow() {
    const domWindow = this.refRoot.map((root) => root.ownerDocument.defaultView).getOr(null);
    if (!domWindow || domWindow === this.surfaceWindow) return;
    this.unbindSurfaceWindow();
    this.surfaceWindow = domWindow;
    this.surfaceWindow.addEventListener("mouseup", this.mouseup);
  }

  unbindSurfaceWindow() {
    this.surfaceWindow?.removeEventListener("mouseup", this.mouseup);
    this.surfaceWindow = null;
  }

  observeSurfaceTransition(context) {
    const root = this.refRoot.getOr(null);
    const itemElement = context.item?.getElement?.();
    if (!root || !itemElement?.contains(root)) return null;
    this.unbindSurfaceWindow();
    const finish = () => this.bindSurfaceWindow();
  ...
  showBulkResolveMenu(event) {
    ...
    this.menuCommandSubs.add(
      lumine.commands.add(target, {
        "git-panel:resolve-all-as-ours": {
          description: "Resolve every conflicted file by keeping our side.",
          didDispatch: () => this.props.resolveAsOurs(conflictPaths),
        },
        "git-panel:resolve-all-as-theirs": {
          description: "Resolve every conflicted file by keeping their side.",
          didDispatch: () => this.props.resolveAsTheirs(conflictPaths),
        },
      }),
    );
    ...
    this.menuCommandSubs.add(
      lumine.commands.add(target, {
        "git-panel:discard-all": {
          description: "Throw away every unstaged change in the working tree.",
          didDispatch: () => this.discardAll({ eventSource: "header-menu" }),
        },
        "git-panel:discard-selected": {
          description: "Throw away the unstaged changes in the selected files.",
          didDispatch: () => this.discardChanges({ eventSource: "header-menu" }),
        },
        "git-panel:undo-last-discard": {
          description: "Bring back the changes the last discard threw away.",
          didDispatch: () => this.undoLastDiscard({ eventSource: "header-menu" }),
        },
      }),
    );
  ...
  async contextMenuOnItem(event, item) {
    if (!this.state.selection.getSelectedItems().has(item)) {
      ...
      const target = event.target;
      const domWindow = target.ownerDocument.defaultView;
      const newEvent = new domWindow.MouseEvent(event.type, event.nativeEvent || event);
  ...
  async mousedownOnItem(event, item) {
    const listKey = this.state.selection.listKeyForItem(item);
    if (event.button === 1 && (listKey === "unstaged" || listKey === "staged")) {
      event.preventDefault();
      event.stopPropagation();
    ...
    if (event.button === 0) {
      ...
      event.persist();
      await new Promise((resolve) => {
        const primaryModifier = event.metaKey || (event.ctrlKey && !darwin);
        if (event.shiftKey && primaryModifier) {
          this.setState(
            (prevState) => ({
              selection: prevState.selection.addRangeToItem(item),
            }),
            resolve,
          );
          ...
          this.setState(
            (prevState) => ({
              selection: prevState.selection.selectRangeToItem(item),
            }),
            resolve,
          );
