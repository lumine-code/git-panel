...
import { Disposable, CompositeDisposable } from "lumine";
...
export default class StagingView extends React.Component {
  ...
  async syncWithWorkspace() {
    ...
    const realItemPromise = item.whenHydrated && item.whenHydrated();
      ...
      if (openNew) {
        ...
        if (panesWithStaleItemsToUpdate.length > 0) {
          ...
          const activePaneHasPendingFilePatchItem =
            activePendingItem &&
            activePendingItem.getHydratedView &&
            activePendingItem.getHydratedView() instanceof ChangedFileItem;
  ...
  getPanesWithStalePendingFilePatchItem() {
    ...
    return this.props.workspace.getPanes().filter((pane) => {
      ...
      if (!pendingItem || !pendingItem.getHydratedView) {
        return false;
      }
      const realItem = pendingItem.getHydratedView();
