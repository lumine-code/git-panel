...
export default class GitTabView extends React.Component {
    ...
    this.props.refRoot.map((root) => {
      ...
      return this.subscriptions.add(
        ...
        this.props.commands.add(root, {
          ...
          "git-panel:focus-project": {
            description: "Move focus to the project section of the panel.",
            didDispatch: this.focusProject,
          },
          "git-panel:focus-unstaged": {
            description: "Move focus to the list of unstaged changes.",
            didDispatch: this.focusUnstaged,
          },
          "git-panel:focus-staged": {
            description: "Move focus to the list of staged changes.",
            didDispatch: this.focusStaged,
          },
          "git-panel:focus-commit": {
            description: "Move focus to the commit message box.",
            didDispatch: this.focusCommit,
          },
          "git-panel:focus-recent-commits": {
            description: "Move focus to the list of recent commits.",
            didDispatch: this.focusRecentCommits,
          },
        }),
      );
    });
  ...
  renderHeader() {
    ...
    return (
      <GitTabHeaderController
        ...
        // The switcher, its option list, and the pin come from lumine.repositories;
    ...
    return (
      <div className="git-panel-Git too-many-changes">
        ...
        <div className="initialize-repo-description">
          ...
          files to display in {lumine.branding.name}. Ensure that you have set up an appropriate{" "}
  ...
  renderUnsupportedDir() {
    return (
      <div className="git-panel-Git unsupported-directory">
        ...
        <div className="initialize-repo-description">
          {lumine.branding.name} does not support managing Git repositories in your home or root
          directories.
        </div>
      </div>
    );
  ...
  clearNoRepoSelectionOnFocusLost() {
    ...
    const domWindow = root?.ownerDocument.defaultView;
    if (!domWindow) return;
  ...
  async advanceFocus(evt) {
    const currentFocus = this.getFocus(
      this.props.refRoot.map((root) => root.ownerDocument.activeElement).getOr(null),
    );
  ...
  abortFromTextEditor(evt) {
    if (!evt.target || !evt.target.closest("lumine-text-editor")) {
      return false;
    }
  ...
  focusAndSelectCommitPreviewButton() {
    // The commit-preview toggle now lives in the staging view's staged header.
    this.props.refStagingView.map((view) => view.focusCommitPreviewButton());
  }
  ...
  hasFocus() {
    return this.props.refRoot
      .map((root) => root.contains(root.ownerDocument.activeElement))
      .getOr(false);
  }
}
