...
const EMPTY_CLASS = "git-panel-AtomTextEditor-empty";
...
export default class AtomTextEditor extends React.Component {
  ...
  render() {
    return (
      <Fragment>
        <div className="git-panel-AtomTextEditor-container" ref={this.refParent.setter} />
  ...
  componentDidMount() {
    ...
    this.refParent.map((element) => {
      ...
      if (this.props.registerWithAtom) {
        this.subs.add(atom.textEditors.add(editor));
      }
