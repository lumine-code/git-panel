...
export default class GitPackage {
  ...
  rerender(callback) {
    ...
    const changeWorkingDirectory = async (workingDirectory) => {
      const repository = await this.repositories.setActiveRepositoryForPath(workingDirectory);
      if (!repository) {
        // Directories without a repository (offering "initialize here") still
        // become the panel's context even though core has nothing to activate.
        await this.scheduleActiveContextUpdate({ usePath: workingDirectory });
      }
    };

    const setContextLock = async (workingDirectory, lock) => {
      const repository = await this.repositories.setActiveRepositoryForPath(workingDirectory, {
        pin: Boolean(lock),
      });
      if (!repository && !lock) {
        this.repositories.setActiveRepository(null);
      }
    };
  ...
  async getNextContext(usePath = null) {
    ...
    // The window's active repository is owned by atom.repositories; the panel
    // only mirrors it into a context.
    const activeWorkdir = getRepositoryWorkingDirectory(this.repositories.getActiveRepository());
    if (activeWorkdir) {
    ...
    if (activeWorkdir) {
      return this.contextPool.getContext(activeWorkdir);
    }
