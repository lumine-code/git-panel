...
export default class GitPackage {
  ...
  async activate(state = {}) {
    ...
    this.subscriptions.add(
      ...
      this.project.onDidAddRepository((atomRepo) => {
        const idx = this.project.getRepositories().indexOf(atomRepo);
        const projectPath = idx !== -1 ? this.project.getPaths()[idx] : null;
        if (!projectPath) return;
        this.workdirCache.invalidate();
        if (!this.internalRepoAdd) {
          this.contextPool.replace(projectPath);
        }
        this.scheduleActiveContextUpdate();
      }),
      this.project.observeRepositories((atomRepo) => {
        const idx = this.project.getRepositories().indexOf(atomRepo);
        const projectPath = idx !== -1 ? this.project.getPaths()[idx] : null;
        return atomRepo.onDidDestroy(() => {
          this.workdirCache.invalidate();
          if (projectPath) this.contextPool.replace(projectPath);
          this.scheduleActiveContextUpdate();
        });
      }),
      ...
      this.commands.add("atom-workspace", {
        "git-panel:update-repositories": async () => {
          ...
          const repos = this.project.getRepositories();
          for (let i = 0; i < paths.length; i++) {
            const workdir = await this.workdirCache.find(paths[i]);
            if (repos[i] != null) {
              if (!workdir) {
                const staleRepo = repos[i];
                repos[i] = null;
                staleRepo.destroy();
              } else {
                try {
                  await repos[i].refreshStatus();
                } catch (_e) {}
              }
            } else if (workdir) {
              const dir = this.project.getDirectoryForProjectPath(paths[i]);
              if (dir) {
                const repo = await this.project.repositoryForDirectory(dir);
                if (repo && repos[i] == null) {
                  repos[i] = repo;
                  this.project.emitter.emit("did-add-repository", repo);
                }
              }
            }
          }
  ...
  handleActivePaneItemChange = () => {
    ...
    if (this.activeItemDebounceTimer) {
      clearTimeout(this.activeItemDebounceTimer);
    }
    this.activeItemDebounceTimer = setTimeout(() => {
      this.activeItemDebounceTimer = null;
      if (this.lockedContext || !this.activated) {
        return;
      }
  ...
  scheduleStartupActiveContextUpdate(options) {
    const schedule = typeof setImmediate === "function" ? setImmediate : setTimeout;
    schedule(() => {
      if (!this.activated || this.workspace.isDestroyed()) {
        return;
      }
      this.scheduleActiveContextUpdate(options);
    }, 0);
  }
  ...
  initializeRepo = async (projectPath) => {
    ...
    const paths = this.project.getPaths();
    const repos = this.project.getRepositories();
    const idx = paths.indexOf(projectPath);
    if (idx !== -1 && repos[idx] == null) {
      const dir = this.project.getDirectoryForProjectPath(projectPath);
      if (dir) {
        const atomRepo = await this.project.repositoryForDirectory(dir);
        if (atomRepo && repos[idx] == null) {
          repos[idx] = atomRepo;
          this.internalRepoAdd = true;
          this.project.emitter.emit("did-add-repository", atomRepo);
          this.internalRepoAdd = false;
        }
      }
    }
