...
class DurationSet {
  constructor(name) {
    ...
    if (atom.config.get("git-panel.performanceToConsole")) {
    ...
    if (atom.config.get("git-panel.performanceToProfile")) {
      // eslint-disable-next-line no-console
  ...
  mark(eventName) {
    ...
    if (atom.config.get("git-panel.performanceToDirectory") !== "") {
...
function shouldCapture(seriesName, eventName) {
  const anyActive = ["Console", "Directory", "Profile"].some((kind) => {
    const value = atom.config.get(`git-panel.performanceTo${kind}`);
  ...
  const mask = new RegExp(atom.config.get("git-panel.performanceMask"));
...
const yardstick = {
  async save() {
    const destDir = atom.config.get("git-panel.performanceToDirectory");
