...
class DurationSet {
  constructor(name) {
    ...
    if (lumine.config.get("git-panel.performanceToConsole")) {
      console.log(
        "%cbegin %c%s:begin",
        "font-weight: bold",
    ...
    if (lumine.config.get("git-panel.performanceToProfile")) {
  ...
  mark(eventName) {
    ...
    if (lumine.config.get("git-panel.performanceToDirectory") !== "") {
...
function shouldCapture(seriesName, eventName) {
  const anyActive = ["Console", "Directory", "Profile"].some((kind) => {
    const value = lumine.config.get(`git-panel.performanceTo${kind}`);
  ...
  const mask = new RegExp(lumine.config.get("git-panel.performanceMask"));
...
const yardstick = {
  async save() {
    const destDir = lumine.config.get("git-panel.performanceToDirectory");
