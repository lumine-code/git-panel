...
export default class ListSelection {
  ...
  setItems(items) {
    ...
    const newSelections =
      items.length > 0 ? [{ head: newSelectionIndex, tail: newSelectionIndex }] : [];
  ...
  selectRangeToItem(item) {
    const itemIndex = this.items.indexOf(item);
    // Shift-click extends from the existing range anchor and replaces disconnected selections.
    const anchor = this.selections[0]?.tail ?? itemIndex;
    return this.copy({ selections: [{ head: itemIndex, tail: anchor }] });
  }

  addRangeToItem(item) {
    const itemIndex = this.items.indexOf(item);
    // Ctrl/Cmd+Shift-click extends from the most recently focused item and retains earlier selections.
    const anchor = this.selections[0]?.head ?? itemIndex;
    return this.copy({
      selections: [{ head: itemIndex, tail: anchor }, ...this.selections],
    });
  }
