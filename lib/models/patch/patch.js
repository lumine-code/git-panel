/** @babel */
import { TextBuffer, Range } from "lumine";
  ...
  buildStagePatchForLines(originalBuffer, nextPatchBuffer, rowSet) {
    ...
    for (const hunk of this.getHunks()) {
      ...
      if (atLeastOneSelectedChange) {
        ...
        const newRowCount =
          marker.getRange().getRowCount() - selectedDeletionRowCount - noNewlineRowCount;
...
class BufferBuilder {
  ...
  markRegion(range, RegionKind) {
    const finalRange =
      this.offset !== 0 ? range.translate([this.offset, 0], [this.offset, 0]) : range;
