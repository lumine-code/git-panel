...
function singleDiffFilePatch(diff, patchBuffer, opts) {
  ...
  if (wasSymlink && !isSymlink) {
    oldSymlink = findDiffLineText(diff, "deleted");
  } else if (!wasSymlink && isSymlink) {
    newSymlink = findDiffLineText(diff, "added");
  } else if (wasSymlink && isSymlink) {
    oldSymlink = findDiffLineText(diff, "deleted");
    newSymlink = findDiffLineText(diff, "added");
  }
  ...
  const renderStatus = renderStatusOverride || (isDiffLarge([diff], opts) && DEFERRED) || EXPANDED;
  ...
  if (!renderStatus.isVisible()) {
    ...
    return FilePatch.createHiddenFilePatch(oldFile, newFile, patchMarker, renderStatus, () => {
...
function dualDiffFilePatch(diff1, diff2, patchBuffer, opts) {
  ...
  const symlink = findDiffLineText(
    modeChangeDiff,
    modeChangeDiff.status === "added" ? "added" : "deleted",
  );
...
function diffLineText(line) {
  return typeof line === "string" ? line.slice(1) : line.text;
}

function diffLineKind(line) {
  if (typeof line !== "string") return line.kind;
  return { "+": "added", "-": "deleted", " ": "context", "\\": "nonewline" }[line[0]];
}

function findDiffLineText(diff, kind) {
  for (const hunk of diff.hunks) {
    const line = hunk.lines.find((candidate) => diffLineKind(candidate) === kind);
    if (line) return diffLineText(line);
  }
  return null;
}
...
function diffLine(line) {
  if (typeof line === "string") {
    return { RegionKind: CHANGEKIND[line[0]], text: line.slice(1), kind: line[0] };
  }

  return {
    RegionKind: STRUCTURED_CHANGEKIND[line.kind],
    // Core represents the no-newline marker semantically. Keep the canonical
    // payload in the patch buffer so partial-stage serialization remains a
    // valid patch accepted by `git apply`.
        ...
        inserter.markWhile(
          ...
          () => {
            ...
            for (const rawLine of rawHunk.lines) {
              const { RegionKind: NextRegionKind, text: nextLine, kind } = diffLine(rawLine);
              if (NextRegionKind === undefined) {
                throw new Error(`Unknown diff line kind: "${kind}"`);
              }
          ...
          },
          {
            ...
            callback: (function (_hunks, _rawHunk, _regions) {
              return (hunkMarker) => {
                _hunks.push(
                  new Hunk({
                    oldStartRow: _rawHunk.oldStartLine ?? _rawHunk.oldStart,
                    newStartRow: _rawHunk.newStartLine ?? _rawHunk.newStart,
                    oldRowCount: _rawHunk.oldLineCount ?? _rawHunk.oldLines,
                    newRowCount: _rawHunk.newLineCount ?? _rawHunk.newLines,
