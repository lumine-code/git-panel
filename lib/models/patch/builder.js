...
function singleDiffFilePatch(diff, patchBuffer, opts) {
  ...
  if (wasSymlink && !isSymlink) {
    oldSymlink = diffLineText(diff.hunks[0].lines[0]);
  } else if (!wasSymlink && isSymlink) {
    newSymlink = diffLineText(diff.hunks[0].lines[0]);
  } else if (wasSymlink && isSymlink) {
    oldSymlink = diffLineText(diff.hunks[0].lines[0]);
    newSymlink = diffLineText(diff.hunks[0].lines[2]);
  }
...
function dualDiffFilePatch(diff1, diff2, patchBuffer, opts) {
  ...
  const symlink = diffLineText(modeChangeDiff.hunks[0].lines[0]);
