...
export default class ModelObserver {
  ...
  async _refreshModelData(model) {
    try {
      ...
      if (fetchDataPromise === this.lastFetchDataPromise && !this.pending) {
        this.activeModel = model;
        this.activeModelData = modelData;
        this.didUpdate(model);
      }
    } catch {
      // fetchData may fail when triggered by cache invalidation after
      // a failed git operation (e.g. push). The operation error is already
      // handled by pipeline middleware, swallow here to prevent unhandled
      // rejections from background refreshes. Stale data will refresh on
