...
import { LargeRepoError } from "../../git-errors";
...
import Commit, { nullCommit } from "../commit";
...
export default class Present extends State {
  constructor(repository, history) {
    ...
    this.fetchInitialMessage().catch((e) => {
  ...
  invalidateCacheAfterFilesystemChange(events) {
    ...
    for (let i = 0; i < paths.length; i++) {
      ...
      if (filePathEndsWith(fullPath, ".git", "HEAD")) {
        ...
        keys.add(Keys.recentCommits.all);
        keys.add(Keys.statusBundle);
      ...
      if (filePathEndsWith(fullPath, ".git", "config")) {
        ...
        // `git submodule add/deinit/update --init` all write .git/config.
        keys.add(Keys.submodules);
        keys.add(Keys.statusBundle);
        continue;
      }

      // Everything else inside .git — reflog appends (.git/logs/HEAD),
      // packed-refs, ORIG_HEAD, COMMIT_EDITMSG — is not a watched cache
      // signal. Without this guard a reflog append fell through below and
      // minted a bogus working-directory filePatch key plus a status-bundle
      // invalidation on every ref move.
      if (fullPath.split(path.sep).includes(".git")) {
        continue;
      }
    ...
    /* istanbul ignore else */
    if (keys.size > 0) {
      if (keys.has(Keys.statusBundle)) {
        // Disk moved without going through lumine.repositories, so core's status snapshot may
        // predate it and the next bundle has to pay for a `git status` of its own.
  ...
  async updateCommitMessageAfterFileSystemChange(events) {
    for (let i = 0; i < events.length; i++) {
      ...
      if (filePathEndsWith(event.path, ".git", "config")) {
        ...
        const template = await this.fetchCommitMessageTemplate();
        // Discard history and identity edits write .git/config too, so only
        // touch the commit box when the template actually changed and the box
        // holds nothing the user typed (the MERGE_HEAD guard above).
        if (this.commitMessageTemplate !== template && this.isCommitMessageClean()) {
          this.setCommitMessage(template || "");
        }
  ...
  observeFilesystemChange(events) {
    ...
    this.updateCommitMessageAfterFileSystemChange(events).catch((e) => {
      if (lumine.config.get("git-panel.debug")) {
        console.error(e);
      }
    });
  ...
  observeStatusChange() {
    this.git().observeStatusChange();
  }

  refresh() {
    // A blanket refresh is asked for precisely when the caller cannot say what changed.
    this.observeStatusChange();
    this.cache.clear();
    this.didUpdate();
  }
  ...
  abortMerge() {
    return this.invalidate(
      () => [Keys.statusBundle, Keys.stagedChanges, Keys.filePatch.all, Keys.index.all],
      async () => {
  ...
  pull(branchName, options = {}) {
    return this.invalidate(
      () => [...Keys.headOperationKeys(), Keys.index.all, Keys.headDescription, Keys.branches],
  ...
  // Internal helper for discards: `git submodule status` spawns per call, and
  // the submodule set only changes through .git/config writes (add, deinit,
  // update --init), which invalidate this key in the watcher above. A
  // .gitmodules-only edit stays stale until a config event or refresh —
  // acceptable for routing discards.
  getSubmodulePaths() {
    return this.cache.getOrSet(Keys.submodules, () => {
      return this.git().getSubmodulePaths();
    });
  }
  ...
  discardWorkDirChangesForPaths(paths) {
    return this.invalidate(
      ...
      async () => {
        const [untrackedFiles, submodulePaths] = await Promise.all([
          this.git().getUntrackedFiles(),
          this.getSubmodulePaths(),
        ]);
        const [filesToRemove, trackedFiles] = partition(paths, (f) => untrackedFiles.includes(f));
        const [filesToUpdateAsSubmodule, filesToCheckout] = partition(trackedFiles, (f) =>
          submodulePaths.includes(f),
        );
        await Promise.all([
          this.git().checkoutFiles(filesToCheckout),
          this.git().updateSubmodules(filesToUpdateAsSubmodule),
          ...filesToRemove.map((filePath) => {
            const absPath = path.join(this.workdir(), filePath);
            return fs.rm(absPath, { recursive: true, force: true });
          }),
        ]);
      },
    );
  ...
  getStatusBundle() {
    return this.cache.getOrSet(Keys.statusBundle, async () => {
      try {
        ...
        if (err instanceof LargeRepoError && lumine.config.get("git-panel.warnLargeRepo")) {
          // Unsolicited, and the status still returns: this reports a degraded
          // result from a background refresh, not a failure of anything asked for.
          lumine.notifications.addHint("Git repository is large", {
            description: `The repository at **${this.workdir()}** has many changed files. Some information may be incomplete.`,
            dismissable: true,
          });
        }
        return {
  ...
  getCommit(sha) {
    return this.cache.getOrSet(Keys.blob.oneWith(sha), async () => {
      ...
      return rawCommit ? new Commit(rawCommit) : nullCommit;
    });
  }

  getRecentCommits(options) {
    return this.cache.getOrSet(Keys.recentCommits.oneWith(options), async () => {
  ...
  getRemotes() {
    return this.cache.getOrSet(Keys.remotes, async () => {
      ...
      return new RemoteSet(remotesInfo.map(({ name, url }) => new Remote(name, url)));
    });
  }
  ...
  async getAheadCount(branchName) {
    ...
    return bundle.branch.aheadBehind ? bundle.branch.aheadBehind.ahead : null;
  }

  async getBehindCount(branchName) {
    const bundle = await this.getStatusBundle();
    return bundle.branch.aheadBehind ? bundle.branch.aheadBehind.behind : null;
  }
