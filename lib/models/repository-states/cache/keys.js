...
export const Keys = {
  ...
  filePatch: {
    ...
    eachWithFileOpts: (fileNames, opts) => {
      ...
      for (let i = 0; i < fileNames.length; i++) {
        for (let j = 0; j < opts.length; j++) {
          keys.push(
            new GroupKey(`file-patch:opt-${Keys.filePatch._optKey(opts[j])}:path-${fileNames[i]}`),
          );
        }
      }
      return keys;
    ...
    eachWithOpts: (...opts) =>
      opts.map((opt) => new GroupKey(`file-patch:opt-${Keys.filePatch._optKey(opt)}`)),
  ...
  recentCommits: {
    oneWith: (options = {}) =>
      new CacheKey(`recent-commits:${options.max || "all"}:${options.skip || 0}`, [
        "recent-commits",
      ]),
    all: new GroupKey("recent-commits"),
  },
  ...
  submodules: new CacheKey("submodules"),
