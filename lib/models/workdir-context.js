...
export default class WorkdirContext {
  ...
  constructor(directory, options = {}) {
    ...
    const { window: theWindow, workspace, pipelineManager } = options;
    this.repository = (
    ...
    this.subs.add(
      ...
      this.observer.onDidInvalidate(() => this.repository.refresh()),
    );
    this.subs.add(
      this.observer.onDidChangeWorkdirOrHead(() => this.emitter.emit("did-change-workdir-or-head")),
    );

    this.coreRepositoryLease = null;
    this.subscribeToCoreRepository();
   ...
   */
  async subscribeToCoreRepository() {
    if (!this.directory) {
      return;
    }
    ...
    this.subs.add(
      coreRepository.onDidChangeStatusSnapshot(() => {
        this.repository.acceptInvalidation(() => [Keys.statusBundle]);
      }),
  ...
  repositoryChangedState(payload) {
    ...
    if (this.repository.isPresent()) {
      this.observer.start().then(
        () => {
          if (!this.destroyed) this.emitter.emit("did-start-observer");
        },
        (error) => {
          if (!this.destroyed) console.warn("Unable to watch repository", error);
        },
      );
  ...
  async destroy() {
    ...
    this.subs.dispose();
    if (this.coreRepositoryLease) {
      this.coreRepositoryLease.dispose();
      this.coreRepositoryLease = null;
    }
