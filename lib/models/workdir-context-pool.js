...
export default class WorkdirContextPool {
  ...
  async getMatchingContext(host, owner, repo) {
    const getRepositoryDirectories = this.options.getRepositoryDirectories;
    if (getRepositoryDirectories) {
      for (const directory of getRepositoryDirectories()) {
        this.add(directory);
      }
    }
  ...
  /**
   * Reconcile a repository that has just been registered by atom.repositories.
   *
