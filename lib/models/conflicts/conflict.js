...
import { EditorAdapter } from "./parser/adapter";

// Regular expression that matches the beginning of a potential conflict.
const CONFLICT_START_REGEX = /^(<{7,}) ([^\r\n]+)\r?\n/g;
const HEADER_LINE_REGEX = /^(<{7,}) (.+)$/;
const BASE_LINE_REGEX = /^(\|{7,}) (.+)$/;
const SEPARATOR_LINE_REGEX = /^(={7,})$/;
const FOOTER_LINE_REGEX = /^(>{7,}) (.+)$/;
...
class ConflictVisitor {
   ...
   */
  constructor(editor, layer, markerSize) {
    this.editor = editor;
    this.layer = layer;
  ...
  visitSeparator(sepRowStart) {
    ...
    const originalText = this.editor.getTextInBufferRange(marker.getBufferRange());
    this.separator = new Separator(this.editor, marker, originalText);
  }
  ...
  sideDescription(bannerRow) {
    const markerPattern = new RegExp(`^[<|>]{${this.markerSize}} (.*)$`);
    return this.editor.lineTextForBufferRow(bannerRow).match(markerPattern)[1];
  }
...
export default class Conflict {
  ...
  static allFromEditor(editor, layer, isRebase) {
    ...
    editor.getBuffer().scan(CONFLICT_START_REGEX, (m) => {
      ...
      const markerSize = m.match[1].length;
      const visitor = new ConflictVisitor(editor, layer, markerSize);
      const parser = new ConflictParser(adapter, visitor, isRebase, markerSize);
   ...
   */
  static async countFromStream(stream) {
    ...
    for await (const line of lines) {
      ...
      const header = HEADER_LINE_REGEX.exec(line);
      if (header && (!current || current.phase === "base")) {
        stack.push({ markerSize: header[1].length, phase: "first" });
        continue;
      }
      if (!current) continue;

      const base = BASE_LINE_REGEX.exec(line);
      if (current.phase === "first" && base && base[1].length === current.markerSize) {
        current.phase = "base";
        continue;
      }

      const separator = SEPARATOR_LINE_REGEX.exec(line);
      if (
        (current.phase === "first" || current.phase === "base") &&
        separator &&
        separator[1].length === current.markerSize
      ) {
        current.phase = "second";
        continue;
      }

      const footer = FOOTER_LINE_REGEX.exec(line);
      if (current.phase === "second" && footer && footer[1].length === current.markerSize) {
        stack.pop();
