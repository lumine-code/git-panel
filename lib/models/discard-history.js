...
export default class DiscardHistory {
  ...
  async storeBeforeAndAfterBlobs(
    ...
    if (partialDiscardFilePath) {
      return await this.storeBlobsForPartialFileHistory(
  ...
  async expandBlobsToFilesInTempFolder(snapshots) {
    const tempFolderPath = await getTempDir({ prefix: "git-panel-discard-history-" });
    const pathPromises = snapshots.map(async ({ filePath, beforeSha, afterSha }) => {
      ...
      const [theirsPath, commonBasePath] = await Promise.all([
        !beforeSha
          ? null
          : this.expandBlobToFile(
              path.join(tempFolderPath, `${filePath}-before-discard`),
              beforeSha,
            ),
        !afterSha
          ? null
          : this.expandBlobToFile(path.join(tempFolderPath, `${filePath}-after-discard`), afterSha),
      ]);
  ...
  async mergeFiles(filePaths) {
    const mergeFilePromises = filePaths.map(async (filePathInfo, i) => {
      const { filePath, commonBasePath, theirsPath, resultPath, theirsSha, commonBaseSha } =
        filePathInfo;
      ...
      if (theirsPath && commonBasePath) {
        ...
        // deleted file; currentSha above already hashed this same working copy
        if (currentSha === commonBaseSha) {
          ...
          await fs.cp(path.join(this.workdirPath, filePath), resultPath);
        ...
        if (!fileDoesExist) {
          await fs.cp(theirsPath, resultPath);
        ...
        throw new Error(
          "One of the following must be defined - theirsPath:" +
