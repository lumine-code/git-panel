...
export default class CompositeListSelection {
  constructor(options) {
    if (options._copy !== COPY) {
      ...
      // Shared by every copy of this selection, so a resolver installed on one
      // copy is visible to whichever copy processes the next list update. A
      // plain field was lost whenever a copy predating the install won the
      // race into React state, leaving the staging guard stuck forever.
      this.updateBox = { resolve: () => {} };
      ...
      this.updateBox = options.updateBox;
    }
  }
  ...
  getNextUpdatePromise() {
    return new Promise((resolve) => {
      this.updateBox.resolve = resolve;
    });
  }
  ...
  activateListKey(listKey) {
    const selection = this.selections.find((each) => this.keysBySelection.get(each) === listKey);
    if (!selection) {
      return this;
    }
    return this.activateSelection(selection);
  }
  ...
  selectRangeToItem(item) {
    ...
    return this.updateActiveSelection((s) => s.selectRangeToItem(item));
  }
  ...
  addRangeToItem(item) {
    ...
    if (selection === this.getActiveSelection()) {
      return this.updateActiveSelection((s) => s.addRangeToItem(item));
    }
    return this.activateSelection(selection).updateActiveSelection((s) => s.selectItem(item));
  }
  ...
  selectNextItem(preserveTail = false) {
    ...
    if (
      !preserveTail &&
      next.getActiveSelection().getHeadItem() === next.getActiveSelection().getLastItem()
    ) {
  ...
  selectPreviousItem(preserveTail = false) {
    ...
    if (
      !preserveTail &&
      next.getActiveSelection().getHeadItem() === next.getActiveSelection().getItems()[0]
    ) {
