...
export default class WorkdirCache {
  ...
  async revParse(startPath) {
    let startDir;
    try {
      startDir = (await fs.stat(startPath)).isDirectory() ? startPath : path.dirname(startPath);
      ...
      const topLevel = await getCompositeGitStrategy()
        .create(startDir)
      ...
      const gitDir = await getCompositeGitStrategy()
        .create(startDir)
        .exec(["rev-parse", "--absolute-git-dir"]);
      ...
      if (e.stdErr && /dubious ownership/.test(e.stdErr)) {
        ...
        const detail = cmdMatch
          ...
          : e.stdErr;
        lumine.notifications.addError("Git: dubious ownership", {
      ...
      /* istanbul ignore if */
      if (lumine.config.get("git-panel.reportCannotLocateWorkspaceError")) {
        console.error(
