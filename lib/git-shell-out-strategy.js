...
export default class GitShellOutStrategy {
  ...
  async getConfig(option, { local } = {}) {
    // A regexp/multi lookup or a local-scoped read (both now unused by the panel)
    // still fall back to a direct git config invocation; single-value reads use
    // the core typed config API.
    if (Array.isArray(option) || local) {
      let output;
      try {
        let args = ["config"];
        if (local) {
