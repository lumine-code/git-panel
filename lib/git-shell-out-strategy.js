...
export default class GitShellOutStrategy {
    ...
    return this.commandQueue.push(
      async () => {
        ...
        if (useGitPromptServer) {
          ...
          env.ATOM_GITHUB_DUGITE_PATH = require.resolve("dugite");
  ...
  executeGitCommand(args, options, marker = null) {
    ...
    const promise = getDugiteExec()(args, this.workingDir, options);
  ...
  updateSubmodules(paths) {
    ...
    return this.exec(["submodule", "update", "--", ...paths.map(toGitPathSep)], {
      writeOperation: true,
    });
  }
  ...
  async createBlob({ filePath, stdin } = {}) {
    ...
    if (filePath) {
      try {
        output = (
          await this.exec(["hash-object", "-w", filePath], { writeOperation: true })
        ).trim();
      } catch (e) {
      ...
      output = (
        await this.exec(["hash-object", "-w", "--stdin"], { stdin, writeOperation: true })
      ).trim();
    } else {
