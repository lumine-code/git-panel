...
export default class GitShellOutStrategy {
  ...
  getCoreRepository() {
    ...
    const registry = this.destroyed ? null : globalThis.atom?.repositories;
  ...
  async exec(args, options = GitShellOutStrategy.defaultExecArgs) {
    ...
    return this.commandQueue.push(
      async () => {
        ...
        let result;
        try {
          result = await atom.repositories.executeGit(args, this.workingDir, opts);
        } finally {
          timingMarker.finalize();
        }
  ...
  clone(remoteUrl, options = {}) {
    ...
    return atom.repositories.clone(remoteUrl, this.workingDir, {
