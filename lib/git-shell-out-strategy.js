...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    this.repositoryOperationsPromise = null;
   ...
   * lease on it so the registry keeps it alive while this strategy delegates operations to it.
   */
  getRepositoryOperations() {
    const lease = this.repositoryLease;
    if (lease) {
      if (!lease.repository.isDestroyed()) {
        return Promise.resolve(lease.repository.getOperations());
      }
      lease.dispose();
      this.repositoryLease = null;
    }

    if (!this.repositoryOperationsPromise) {
      this.repositoryOperationsPromise = atom.repositories
        ...
        .then((acquired) => {
          ...
          return acquired.repository.getOperations();
        })
        .catch((error) => {
          this.repositoryOperationsPromise = null;
          throw error;
    ...
    return this.repositoryOperationsPromise;
  }
            ...
            gitPromptServer.onDidCancel(async ({ handlerPid }) => {
              ...
              if (childPid !== null) {
                await new Promise((resolveKill) => {
                  require("tree-kill")(childPid, "SIGTERM", () => resolveKill());
                });
              }
  ...
  async getDiffsForFilePath(filePath, { staged, baseCommit } = {}) {
    ...
    let rawDiffs = [];
    if (output) {
      rawDiffs = parseDiff(output).filter((rawDiff) => rawDiff.status !== "unmerged");
  ...
  async getCommits(options = {}) {
    ...
    const fields = output.trimStart().split("\0");
  ...
  async getSubmodulePaths() {
    const output = await this.exec(["submodule", "status"]);
    if (output.trim() === "") {
      return [];
    }
    return output
      .trim()
      .split(LINE_ENDING_REGEX)
      .map((line) => toNativePathSep(line.trim().split(/\s+/)[1]))
      .filter(Boolean);
  }
