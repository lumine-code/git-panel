...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    this.postOperationStatusSnapshotGeneration = null;
  }
  ...
  runRepositoryOperation(operationName, invoke, options = {}) {
    const { refreshesStatus, writeOperation } = options;
        ...
        try {
          ...
          if (refreshesStatus) {
            ...
            if (snapshot?.initialized) {
              this.postOperationStatusSnapshotGeneration = snapshot.generation;
            }
          }
          return result;
        } catch (error) {
  ...
  stageFiles(paths) {
    ...
    return this.runRepositoryOperation(
      ...
      { writeOperation: true, refreshesStatus: true },
    );
  }
  ...
  async getStatusBundle() {
    ...
    // after every delegated write, and core consumers refresh it on their own triggers. A panel
    // operation can invalidate its cache after the core event was already consumed, so remember
    // that post-operation generation and reuse it for the second rebuild too.
    const current = repository.getStatusSnapshot();
    let snapshot;
    const isPostOperationSnapshot =
      current.initialized && current.generation === this.postOperationStatusSnapshotGeneration;
    if (
      current.initialized &&
      (current.generation !== this.lastStatusSnapshotGeneration || isPostOperationSnapshot)
    ) {
      snapshot = current;
      if (isPostOperationSnapshot) {
        this.postOperationStatusSnapshotGeneration = null;
      }
