...
// Adapt core's structured diff file records to the path and status conventions
// used by git-panel's patch model. Hunks and lines remain structured objects;
// the patch builder consumes them directly, avoiding a raw-patch round trip.
function panelDiffsFromStructuredFiles(files) {
  return files.map((file) => ({
    ...file,
    // The legacy parser classified a one-record type change as modified. The
    // old/new modes still let FilePatch identify and serialize the type change.
    status: file.status === "typechange" ? "modified" : file.status,
    oldPath: file.oldPath == null ? null : toNativePathSep(file.oldPath),
    newPath: file.newPath == null ? null : toNativePathSep(file.newPath),
  }));
}
...
export default class GitShellOutStrategy {
  ...
  constructor(workingDir, options = {}) {
    ...
    // Changes core's status snapshot may not have seen, and how many of them
    // the snapshot the panel last built from is known to cover. See
    // {::observeStatusChange} and {::getStatusBundle}.
    this.observedStatusChangeCount = 0;
    this.coveredStatusChangeCount = 0;
  }

  /*
   * Record that something may have changed the repository behind core's back: a filesystem event,
   * or a working-directory write the panel made around the repository model. The next status
   * bundle refreshes core's snapshot instead of reusing it.
   *
   * Writes delegated to lumine.repositories are not such a change — the registry refreshes the
   * snapshot before the operation resolves — and neither is a cache invalidation core's own
   * snapshot-change event triggered, since that snapshot is what the event announced.
   */
  observeStatusChange() {
    this.observedStatusChangeCount++;
  }

  /*
   * Resolve the Lumine repository for this working directory through lumine.repositories and hold a
   * lease on it so the registry keeps it alive while this strategy reads from it and delegates
   * operations to it.
   */
  getCoreRepository() {
    // The environment drops its registry when the window closes or reloads, and
    // a reload does not deactivate packages first, so reads still in flight can
    // outlive it. Treat a missing registry like a destroyed strategy: every
    // caller here already handles "no repository".
    const registry = this.destroyed ? null : globalThis.lumine?.repositories;
    ...
    if (!this.coreRepositoryPromise) {
      this.coreRepositoryPromise = registry
  ...
  // The base endpoint for a staged diff: HEAD normally, or an algorithm-neutral
  // empty tree before the first commit.
  async stagedBaseEndpoint(repository) {
    ...
    return !head || head.unborn ? { type: "empty" } : { type: "commit", revision: "HEAD" };
  }

  /*
   * Run a repository write operation through the lumine.repositories facade. The registry serializes
  ...
  runRepositoryOperation(operationName, invoke, options = {}) {
    ...
    return this.commandQueue.push(
      async () => {
        ...
        // Credentials, safe.directory, GIT_TERMINAL_PROMPT and the git binary
        // are all owned by the core operation provider and its auth broker, so
        // the panel passes no environment of its own.
        timingMarker.mark("execute");
        try {
          return await invoke(operations, {});
        } catch (error) {
  ...
  translateRepositoryOperationError(error, formattedOperation) {
    ...
    if (error.code && error.code !== "ERR_GIT_COMMAND_FAILED") {
      return error;
    }
  ...
  // Execute a git command through the core worker and read its stdout. Used for
  // repository creation/discovery (init/clone/rev-parse), effective config
  // reads outside a worktree, and by specs; repository-backed data reads use
  // typed core APIs. Color is disabled here because the raw exec path does not
  // apply the worker's color config.
  async exec(args, options = GitShellOutStrategy.defaultExecArgs) {
    const { stdin, writeOperation } = options;
    ...
    return this.commandQueue.push(
      async () => {
        ...
        let result;
        try {
          result = await lumine.repositories.executeGit(args, this.workingDir, opts);
        } finally {
          timingMarker.finalize();
        }
  ...
  async getStatusBundle() {
    ...
    // Reuse core's snapshot unless something has happened that it may not cover. The registry
    // refreshes it after every delegated write, and core refreshes it on its own triggers, so a
    // rebuild is usually reacting to a snapshot that is already the freshest one there is —
    // one operation alone invalidates this cache twice, once for the write and once for the
    // snapshot-change event it produces. Only {::observeStatusChange} makes a refresh necessary.
    //
    // The snapshot's `generation` cannot stand in for this: core keeps it (and emits no change
    // event) when a refresh finds `git status` unchanged, so comparing it against the last
    // bundle's would buy a subprocess for every rebuild in a quiet repository.
    const current = repository.getStatusSnapshot();
    let snapshot;
    if (current.initialized && this.coveredStatusChangeCount === this.observedStatusChangeCount) {
      snapshot = current;
    } else if (this.pendingStatusRefresh) {
      // Concurrent rebuilds — an invalidation round racing the watcher round —
      // share one awaited refresh. Filesystem and core observers request their
      // own trailing refresh when a genuinely newer change arrives.
      snapshot = await this.pendingStatusRefresh;
    } else {
      // A change observed while `git status` runs is not covered by its output, so the
      // watermark records what was known when the subprocess started, never when it ended.
      const covered = this.observedStatusChangeCount;
      this.pendingStatusRefresh = repository.refreshStatusSnapshot().finally(() => {
        this.pendingStatusRefresh = null;
      });
      snapshot = await this.pendingStatusRefresh;
      this.coveredStatusChangeCount = Math.max(this.coveredStatusChangeCount, covered);
    }
  ...
  async diffEndpoints(repository, { staged, target } = {}) {
    if (staged) {
      return {
        from: await this.stagedBaseEndpoint(repository),
        to: { type: "index" },
      };
    }
    if (target) {
  ...
  async diffFileStatus(options = {}) {
    ...
    const { files } = await repository.getDiff({
      from,
      to,
      detectRenames: false,
      format: "structured",
    ...
    for (const file of files) {
      const filePath = toNativePathSep(file.newPath || file.oldPath);
      fileStatuses[filePath] = statusMap[file.status] || file.status;
    }
    if (!options.staged) {
  ...
  async getDiffsForFilePath(filePath, { staged, baseCommit } = {}) {
    ...
    const rawDiffs = panelDiffsFromStructuredFiles(files).filter(
  ...
  async getStagedChangesPatch() {
    ...
    return panelDiffsFromStructuredFiles(files);
  }
  ...
  async getCommits(options = {}) {
    ...
    // Core represents both an unborn repository and an unknown revision as an
    // empty history page. Consult HEAD only when the caller needs the legacy
    // unborn sentinel so an invalid ref is not mislabeled as an unborn branch.
    ...
    if (records.length === 0) {
      if (!includeUnborn) return [];
      const current = repository.getStatusSnapshot();
      const status = current.initialized ? current : await repository.ensureStatusSnapshot();
      return status.head?.unborn ? unborn() : [];
    }
    ...
    return Promise.all(
      records.map(async (record) => {
        ...
        if (includePatch) {
          ...
          // an algorithm-neutral empty endpoint for a root commit.
          const from = record.parents.length
            ? { type: "commit", revision: record.parents[0] }
            : { type: "empty" };
          const { files } = await repository.getDiff({
            from,
          ...
          patch = panelDiffsFromStructuredFiles(files);
        }
  ...
  async readFileFromIndex(filePath) {
    const repository = await this.getCoreRepository();
    return repository ? repository.getIndexFile(filePath) : null;
  }
  ...
  checkoutSide(side, paths) {
    ...
    return this.runRepositoryOperation(
      "checkout",
      (operations, options) => operations.checkoutSide(side, gitPaths, options),
      { writeOperation: true },
    );
  }
  ...
  clone(remoteUrl, options = {}) {
    // Clone through the registry so it runs in the worker with the core auth
    // broker's credential environment, matching fetch/pull/push.
    return lumine.repositories.clone(remoteUrl, this.workingDir, {
      noLocal: options.noLocal,
      bare: options.bare,
      recursive: options.recursive,
      sourceRemoteName: options.sourceRemoteName,
    });
  }
  ...
  reset(type, revision = "HEAD") {
    ...
    return this.runRepositoryOperation(
      "reset",
      (operations, options) => operations.reset(type, revision, options),
      { writeOperation: true },
    );
  }
  ...
  deleteRef(ref) {
    return this.runRepositoryOperation(
      "update-ref",
      (operations, options) => operations.deleteRef(ref, options),
      { writeOperation: true },
    );
  }
  ...
  async getConfig(option) {
    ...
    // Loading and absent Repository states intentionally read effective
    // global/system config from a filesystem root. There is no core Repository
    // to lease there, but `git config` itself does not require a worktree.
    try {
      const value = await this.exec(["config", "-z", "--get", option]);
  ...
  addRemote(name, url) {
    return this.runRepositoryOperation(
      "remote",
      (operations, options) => operations.addRemote(name, url, options),
      { writeOperation: true },
    );
  }

  // Not a writeOperation: hash-object only adds loose objects, invisible to
  // every panel read, and the core registry still serializes actual execution
  // per repository. Keeping blob creation parallel-friendly lets the discard
  // machinery batch its snapshots instead of draining the whole queue.
  async createBlob({ filePath, stdin } = {}) {
    let output;
    if (filePath) {
      try {
        output = (
          await this.runRepositoryOperation("hash-object", (operations, options) =>
            operations.createBlob({ ...options, filePath }),
          )
        ).trim();
      } catch (e) {
        if (e?.code === "ERR_GIT_CREATE_BLOB") {
          output = null;
        } else {
          throw e;
        }
      ...
      output = (
        await this.runRepositoryOperation("hash-object", (operations, options) =>
          operations.createBlob({ ...options, stdin }),
        )
      ).trim();
    } else {
