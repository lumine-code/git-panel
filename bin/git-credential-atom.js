...
async function fromKeytar(query) {
  ...
  if (password === UNAUTHENTICATED) {
    ...
    const githubHost =
      query.host === 'github.com' ? `${query.protocol}://api.${query.host}` : `${query.protocol}://${query.host}`;
  ...
  if (password !== UNAUTHENTICATED) {
    const lines = ['protocol', 'host', 'username'].filter(k => query[k] !== undefined).map(k => `${k}=${query[k]}\n`);
